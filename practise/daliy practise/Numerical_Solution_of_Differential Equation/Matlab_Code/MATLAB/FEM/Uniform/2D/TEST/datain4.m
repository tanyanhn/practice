...
%--------------------------------------------------------------------%
  ...
  a=0; b=1;  c=0; d=1;   	% Rectangular domain: a <= x <= b,
				%		      c <= y <= d.
  nx = 20; ny = 20;		% Number of grid points in x, y direction.
...
%  npres = 2*(ny-1) + 2*(nx+1);
  ...
  for i=0:ny,       			      % The left edge.
     npt(i+1) = i*(nx +1) + 1;
     xt = x(npt(i+1));  yt = y(npt(i+1)); 
     g1(i+1) = exp(xt)*cos(pi*yt);
  end

  for i=0:ny,				      % The left edge.
     npt(ny+1+i+1) = i*(nx +1) + 1 + nx;
     xt = x(npt(ny+1+i+1));  yt = y(npt(ny+1+i+1));
     g1(ny+1+i+1) = exp(xt)*cos(pi*yt);
  end
