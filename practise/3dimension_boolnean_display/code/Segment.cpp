...
bool Segment::operator<(const Segment& seg2) const {
    Point p1, p2;
    if(Tol::l->ifoverlapLine(*this) == false)
        p1 = Tol::l->intersectionLine(*this);
    else
        p1 = Data::points[this->points[1]];
    if(Tol::l->ifoverlapLine(seg2) == false)
        p2 = Tol::l->intersectionLine(seg2);
    else
        p2 = Data::points[seg2.points[1]];
    if(p1 == p2){
        Direction d1, d2, d = Tol::l->getdirect();
        if(p1 == Data::points[(*this)[0]]){
            d1 = Data::points[(*this)[1]] - Data::points[(*this)[0]];
        }
        else{
            d1 = Data::points[(*this)[0]] - Data::points[(*this)[1]];
        }
        if(p2 == Data::points[seg2[0]]){
            d2 = Data::points[seg2[1]] - Data::points[seg2[0]];
        }
        else{
            d2 = Data::points[seg2[0]] - Data::points[seg2[1]];
        }
        double x = d.angle(d1, *Tol::outside),
            y = d.angle(d2, *Tol::outside);
        return x < y;
    }
    return p1 < p2;
}