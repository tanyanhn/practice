...
class Point {
 ...
 public:
  explicit Point(const double i0 = 0,
                 const double i1 = 0,
                 const int ident = -1) {
    coord[0] = i0;
    coord[1] = i1;
  ...
  bool operator==(const Point& p) const; /*{
      if(fabs(coord[0] - p[0]) < Sweepline::tol && fabs(coord[1] - p[1]) <
  Sweepline::tol) return true; else return false;
  }*/
  bool operator<(const Point& p) const;  /*{
       if(coord[1] < p[1] - Sweepline::tol){
           return true;
       }
       else if(coord[1] < p[1] + Sweepline::tol && coord[0] < p[0] -
       Sweepline::tol) {  return true;
       }
       return false;
       }*/
