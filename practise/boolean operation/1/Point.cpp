...
/*class GraphLine;
  ...
  Point& operator=(Point& p){ coord[0] = p[0]; coord[1] = p[1];}
...
/*  class GraphPoint : public Point {
  ...
  public:*/
GraphPoint::GraphPoint(double d1, double d2) : Point(d1,d2) {
    set<GraphLine*> nearl;
    //sweepLine::eventpoint.add(*this);
}
GraphPoint::GraphPoint(const GraphPoint& p) : Point(p){
    set<GraphLine*> nearl;
    if(!p.nearline().empty()){
...
GraphPoint& GraphPoint::operator=(const GraphPoint& p) {
    coord[0] = p[0];
    coord[1] = p[1];
    set<GraphLine*>::iterator it;
...
GraphPoint::~GraphPoint(){ //sweepLine::eventpoint.del(*this);
    nearl.erase(nearl.begin(), nearl.end()); }
double& GraphPoint::operator[](int i){
    if((i < 0)||(i > 1)) {cout << "Point[] wrong!"; exit(0) ;}
    return coord[i];
}
const double& GraphPoint::operator[](int i) const {
    if((i < 0)||(i > 1)) {cout << "Point[] wrong!"; exit(0) ;}
    return coord[i];
}
bool GraphPoint::operator<(const GraphPoint& p) const {
                                                        if(coord[1] < p.coord[1]) return true;
                                                        else if(coord[1] > p.coord[1]) return false;
                                                        else if(coord[0] < p.coord[0]) return true;
                                                        else return false;
                                                        }
...
GraphLine* GraphPoint::findnextline(GraphLine* l){
    GraphLine* lp = findsmallangle(l, 1);
    return lp;
}
GraphLine* GraphPoint::findprevline(GraphLine* l){
    GraphLine* lp = findsmallangle(l, 2);
}
GraphLine* GraphPoint::findbrline(GraphLine* l){
    if(this != (*l).ep[0]) {
...
const set<GraphLine*>& GraphPoint::nearline() const { return nearl;}
set<GraphLine*>& GraphPoint::nearline(){ return nearl;}
void GraphPoint::addnearl(GraphLine* l){ nearl.insert(l); }
void GraphPoint::remnearl(GraphLine* l){ nearl.erase(l); }
//GraphLine* GraphPoint::findsmallangle(GraphLine* l, int o);  // when o = 1, find next
